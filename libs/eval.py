...
def calculate_metric(y_true, y_pred, y_prob, tasktype, datatype, prob=False):
    ...
    if tasktype == "regression":
        ...
        y_true_classes = np.argmax(y_true, axis=1) if y_true.ndim == 2 and y_true.shape[1] > 1 else y_true.reshape(-1)
        y_pred = np.argmax(y_pred, axis=1) if y_pred.ndim == 2 and y_pred.shape[1] > 1 else y_pred.reshape(-1)
...
def log_training_options(fname, file_path="error.log"):
    with open(file_path, "a") as file:
        file.write(f'Error raised. Unbalanced class problem. ({fname})\n')

def check_if_fname_exists_in_error(fname, file_path="error.log"):
    result = True
    try:
