        ...
        *,
        ...
        if attentiontype == 'col':
            self.transformer = Transformer(
                num_tokens=self.total_tokens, dim=dim, depth=depth, heads=heads,
                dim_head=dim_head, attn_dropout=attn_dropout, ff_dropout=ff_dropout
            )
        elif attentiontype in ['row', 'colrow']:
            self.transformer = RowColTransformer(
                num_tokens=self.total_tokens, dim=dim, nfeats=nfeats, depth=depth,
                x_num=self.x_num, x_cat=self.x_cat, heads=heads, dim_head=dim_head,
                attn_dropout=attn_dropout, ff_dropout=ff_dropout, style=attentiontype
            )
...
class build_saint(SAINT):
    def __init__(self,
                 categories, num_continuous, categories_index, dim, depth, heads, dim_head=16, dim_out=1,
                 mlp_hidden_mults=(4, 2), mlp_act=None, num_special_tokens=0, attn_dropout=0., ff_dropout=0.,
                 cont_embeddings='MLP', scalingfactor=10, attentiontype='colrow', final_mlp_style='common',
                 y_dim=2, optimizer="AdamW", learning_rate=0., weight_decay=0.):
        super().__init__(categories=categories, num_continuous=num_continuous, categories_index=categories_index,
                         dim=dim, depth=depth, heads=heads, dim_head=dim_head, dim_out=dim_out,
                         mlp_hidden_mults=mlp_hidden_mults, mlp_act=mlp_act, num_special_tokens=num_special_tokens,
                         attn_dropout=attn_dropout, ff_dropout=ff_dropout, cont_embeddings=cont_embeddings,
                         scalingfactor=scalingfactor, attentiontype=attentiontype,
                         final_mlp_style=final_mlp_style, y_dim=y_dim)
...
class main_saint(supmodel):
    def __init__(self, params, tasktype, num_cols=[], categories=[], categories_index=[], input_dim=0, output_dim=0, device='cuda', data_id=None, modelname="saint"):
        ...
        self.model = build_saint(
            categories, num_cols, categories_index, embedding_dim, params["depth"], params["heads"], params["hidden"],
            mlp_hidden_mults=(4, 2), mlp_act=params["activation"], attn_dropout=attn_dropout, ff_dropout=ff_dropout,
            cont_embeddings=params.get("cont_embeddings", "MLP"), attentiontype=params.get("attentiontype", "colrow"),
            final_mlp_style=params["final_mlp_style"], y_dim=output_dim,
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        if self.params["lr_scheduler"]:
            scheduler = CosineAnnealingLR_Warmup(
                optimizer, base_lr=self.params['learning_rate'], warmup_epochs=10,
    ...
    def predict(self, X_test):
        ...
        with torch.no_grad():
            test_dataset = CustomTensorDataset(
                X=X_test, y=np.zeros(len(X_test)),
                cat_cols=self.categories, con_cols=self.num_cols, task=self.tasktype
            )
            if (X_test.shape[0] > 10000) or (X_test.shape[1] > 240):
                ...
                cat_features, cont_features, _ = test_dataset[:]
