...
def get_search_space(trial, modelname, num_features=None, data_id=None):
    if modelname == "randomforest":
        ...
        params = {
            ...
            'max_features': trial.suggest_categorical('max_features', [int(np.sqrt(num_features)), max(1, int(np.log2(num_features))), 0.5, 0.75, 1.0]),
            'n_estimators': 300
        }
...
def suggest_initial_trial(modelname):
    init_values = {
        ...
        "catboost": {"learning_rate": 0.05, "max_depth": 6, "l2_leaf_reg": 3, "max_ctr_complexity": 4}, # TabRepo
...
def rearrange_params(modelname, data_id, params, num_features=None):
    if modelname in ["lr", "tabpfn", "knn", "dt"]:
        return {}
    trial = optuna.trial.FixedTrial(dict(params))
    return get_search_space(trial, modelname, num_features=num_features, data_id=data_id)
