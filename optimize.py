...
def main():
    ...
    parser.add_argument("--seed", type=int, choices=range(10), default=1, help="seed for dataset split (cross-validation)")
    ...
    if Path(args.savepath).name != "optim_logs":
    ...
    if os.path.exists(fname):
        ...
        check_implementation(study.user_attrs, revision, args.allow_legacy_logs)
        ...
        study.set_user_attr("implementation_id", revision)
    ...
    if train:
        ...
        def objective(trial):
            ...
            if tasktype == "regression":
                val_metrics = calculate_metric(y_val*y_std, preds_val*y_std, probs_val, tasktype, 'val', prob=True)
                test_metrics = calculate_metric(y_test*y_std, preds_test*y_std, probs_test, tasktype, 'test', prob=True)
            else:
                val_metrics = calculate_metric(y_val, preds_val, probs_val, tasktype, 'val', prob=True)
                test_metrics = calculate_metric(y_test, preds_test, probs_test, tasktype, 'test', prob=True)
