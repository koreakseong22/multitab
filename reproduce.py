...
def is_study_todo(study, tasktype, optimal_value=1.0, num_trials=100):
    ...
    if tasktype != "regression":
        if any(t.state == optuna.trial.TrialState.COMPLETE for t in study.trials) and study.best_value >= optimal_value:
...
def main():
    ...
    i = 0
    with open(error_fname, "r") if os.path.exists(error_fname) else nullcontext([]) as file:
        for line in file:
            l = line.replace("\\", "/").split("optim_logs/")[-1]
    ...
    for m in models:
        for (init_hps, ensemble_deep, ensemble_hyper) in opt_dict[m]:
            ...
            print("##########################################")
            if todo:
                if m == "tabpfn" and (X_train.size(0) > 3000 or tasktype == "regression"):
                ...
                if m not in ["tabpfn", "lr"]:
                    ...
                    params = rearrange_params(m, args.openml_id, params, num_features=X_train.shape[1])
                ...
                if (tasktype == "multiclass") & (m in ["catboost", "xgboost", "lightgbm"]):
                    if torch.any(y_train.sum(dim=0) == 0):            
                        np.save(fname, "ValueError: Not implemented.") # "Unknown class problem" --- Inherent challenges in GBDTs
                        continue
                ...
                if tasktype == "regression":
                    ...
                    metric_prob = model.predict_proba(X_test)
                    test_metrics = calculate_metric(y_test, preds_test, metric_prob, tasktype, 'test', prob=True)
